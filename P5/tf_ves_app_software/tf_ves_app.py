# tf_ves_app.py  (versión modificada: cálculo HR en tiempo real)
...
class TFVesGUI:
    ...
    def __init__(self):
        ...
        # ahora df_adc incluye también timestamps 't'
        self.df_adc = pd.DataFrame(columns=['adc','t'])
        ...
        # ventana de timestamps paralela para mapear índices de picos a tiempos
        self._adc_times = [0.0] * self.adc_window_size
    ...
    def update_adc_plot_from_df_adc_non_accum(self):
        ...
        # sincronizar timestamps: df_adc debe contener columna 't'
        if 't' in self.df_adc.columns:
            new_times = self.df_adc['t'].iloc[:n_plot].tolist()
        else:
            new_times = [time.time()] * n_plot

        # actualizar ventana de valores y ventana de tiempos en paralelo
        self._adc_window = self._adc_window[n_plot:] + new_vals
        # shift times and append the new_times at the end
        self._adc_times = self._adc_times[n_plot:] + new_times
        ...
        # calcular HR a partir de la ventana ADC actualizada
        try:
            self._compute_hr_from_adc_window()
        except Exception as e:
            # fallo no crítico: mostrar en consola
            # No lanzar excepción para no romper el loop
            print("Error calculando HR:", e)
    ...
    def adc_notification_handler(self, sender, data):
        samples = data
        # generar timestamps aproximados para cada muestra (mejor que ninguno)
        times = [time.time() for _ in samples]
        # convertir a voltios
        new_data = pd.DataFrame({'adc': samples, 't': times})
        try:
            new_data['adc'] = (new_data['adc'] / ADC_RESOLUTION_STEPS) * ADC_REFERENCE_VOLTAGE
        except Exception:
            # si las constantes no están definidas correctamente, dejar como están
            pass

        # append to df para graficar (ahora con columna 't')
        if self.df_adc.empty:
        ...
        for val, now in zip(new_data['adc'].tolist(), new_data['t'].tolist()):
            # actualizar último ADC conocido
            self.last_values['adc'] = float(val)

            # actualizar ventana de timestamps (paralela)
            # shift left y agregar now
            self._adc_times = self._adc_times[1:] + [now]
    ...
    # df_window es un DataFrame con columnas acc_x,acc_y,acc_z,gyro_x,gyro_y,gyro_z
        ...
        tmp['label'] = 0 # dummy
        return tmp
    def classify_recent_window(self, fs=50, window_s=2.56):
        """
    ...
    def create_gui(self):
        with dpg.window(label="TF_VES", tag="win", autosize=True) as primary_window:
            ...
            with dpg.theme(tag="spacing"):
                with dpg.theme_component(dpg.mvAll):
                    dpg.add_theme_style(dpg.mvStyleVar_ItemSpacing, -50, 2)

            with dpg.group(horizontal=True, tag="main_layout"):
                dpg.bind_item_theme("main_layout", "spacing")
                # Controles a la izquierda
                with dpg.group(horizontal=False, label="Controls", width=200):
                    ...
                    dpg.add_text("Ajuste de Resistencia")

                    dpg.add_combo(
                        items=["390", "943","2050", "10000"],
                        default_value=str(self.default_Rpot),
                        width=300,
                        callback=self.update_gain,
                        tag="R_pot_selector"
                    )
                    dpg.add_text("Ganancia G")
                    ...
                    dpg.add_input_text( tag="record_filename", default_value="datos_medidos", width=260)

                    # Guardar últimos N samples (botón)
                    dpg.add_spacing(count=1)
                    dpg.add_input_int(label="N", tag="save_last_n", default_value=500, min_value=1, width=200)
                    dpg.add_button(label="Guardar últimos N samples", callback=lambda s, a, u: self.save_last_n_samples(dpg.get_value("save_last_n")))

                    dpg.add_separator()
                    dpg.add_text("Guardar por índices de sample")
                    dpg.add_input_int(label="Inicial", tag="save_idx_start", default_value=0, min_value=0, width=200)
                    dpg.add_input_int(label="Final", tag="save_idx_end", default_value=1000, min_value=0, width=200)
                    dpg.add_button(label="Guardar por índices", callback=lambda s, a, u: self.save_by_index_range(dpg.get_value("save_idx_start"), dpg.get_value("save_idx_end")))
                    ...
                    dpg.add_spacer(height=10) 
                    dpg.add_text("Frecuencia cardíaca:")
                    dpg.add_input_text(label="", tag="hr_display", default_value="-- BPM", readonly=True, width=200)
                    dpg.add_spacer(height=6)  
                    ...
                    dpg.add_input_text(tag="filter_status", default_value="DESCONOCIDO", readonly=True, width=180)
                    dpg.add_spacer(height=6)
                ...
                with dpg.group(horizontal=False, width=600):
                    ...
                    for axis, label in imu_axes_list:
                        with dpg.plot(label=f"IMU {axis.upper()}", height=150, width=600, tag=f"acc_plot_{axis}"):
                    ...
                    with dpg.group(horizontal=False):
                        with dpg.plot(label="ADC", height=200, width=1000, tag="adc_plot"):
    ...
    def run(self):
        ...
        dpg.create_viewport(title='Taller Fourier Vestible', width=1500, height=850)
        self.create_gui()
    ...
    # ------------------------------------------------------------
    def _compute_hr_from_adc_window(self):
        """
        Detecta picos positivos en self._adc_window, mapea índices a timestamps en self._adc_times,
        calcula diferencias T entre picos y devuelve frecuencia f = 1/T (Hz) y BPM = 60 * f.
        Actualiza widget 'hr_display'.
        """
        arr = np.array(self._adc_window, dtype=float)
        times = np.array(self._adc_times, dtype=float)

        # requisitos mínimos
        if len(arr) < 3 or np.allclose(arr, 0.0):
            return

        # remover offset lento (mediana)
        arr_d = arr - np.median(arr)

        # umbral dinámico para evitar capturar ruido
        std = np.std(arr_d)
        mean = np.mean(arr_d)
        if std <= 0:
            return

        # threshold: mean + 0.4*std (ajustable)
        height_th = mean + 0.4 * std
        # prominencia mínima relativa
        prominence = max(0.15 * std, 1e-4)
        # distancia mínima entre picos en muestras -> evitar detectar subpicos. Asumo mínimo 0.4s entre latidos.
        min_distance_samples = int(max(1, int(0.4 * self.adc_freq_hz)))

        peaks, props = find_peaks(arr_d, height=height_th, prominence=prominence, distance=min_distance_samples)

        if len(peaks) < 2:
            # no hay suficientes picos detectados -> mostrar vacío o mantener anterior
            # ponemos "--" si no hay historial
            if self.hr_bpm == 0.0:
                try:
                    dpg.set_value("hr_display", "-- BPM")
                except Exception:
                    pass
            return

        # obtener timestamps de picos (filtrando timestamps válidos)
        peak_times = times[peaks]
        # eliminar ceros o tiempos inválidos
        peak_times = peak_times[peak_times > 0.0]

        if len(peak_times) < 2:
            return

        # calcular diferencias entre picos (inter-beat intervals)
        diffs = np.diff(peak_times)
        # filtrar valores atípicos (picos dobles etc.) - conservador: mantener diffs razonables (0.3s .. 2.0s)
        diffs = diffs[(diffs > 0.25) & (diffs < 2.5)]
        if len(diffs) == 0:
            return

        # usar mediana de intervalos para robustez
        T_med = float(np.median(diffs))
        if T_med <= 0:
            return
        ...
        # guardar en estado
        self.hr_bpm = bpm
        self.hr_freq_hz = freq_hz

        # actualizar GUI (redondeo)
        display_text = f"{bpm:.1f} BPM ({freq_hz:.2f} Hz)"
        try:
            dpg.set_value("hr_display", display_text)
        except Exception:
            pass
